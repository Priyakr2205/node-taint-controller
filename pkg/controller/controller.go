...
const workerPoolLabel = "node.kubernetes.io/worker-pool"
...
func (c *Controller) setNodeLabel(node *corev1.Node) error {
	...
	klog.Info("cluster name ::", cluster_name)
	klog.Info("****************************************************")
        klog.Info("node name ::", node.Name)
	klog.Info("*****************************************************")

        tmp1:= strings.TrimLeft(node.Name,cluster_name)
	klog.Info("tmp string 1 ::", tmp1)
	klog.Info("*************************************************************")

	index:= strings.Index(tmp1,"-")
	klog.Info("index::", index)
       
	if(index != -1){
		workerpool:= tmp1[:index]
		klog.Info("workerpool::", workerpool)
		node.Labels[workerPoolLabel] = workerpool
...
func (c *Controller) handleNode(obj interface{}) {
	...
	if _, nodeLabelAlreadySet := node.Labels[workerPoolLabel]; nodeLabelAlreadySet {
		return
	}
